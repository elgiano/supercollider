...
struct SC_AlsaMidiClient {
    ...
    int connectInput(int inputIndex, int uid, int (*action)(snd_seq_t*, snd_seq_port_subscribe_t*),
                     const char* actionName);
    int connectOutput(int outputIndex, int uid, int (*action)(snd_seq_t*, snd_seq_port_subscribe_t*),
                      const char* actionName);
...
int SC_AlsaMidiClient::connectInput(int inputIndex, int uid, int (*action)(snd_seq_t*, snd_seq_port_subscribe_t*),
                                    const char* actionName) {
...
int SC_AlsaMidiClient::connectOutput(int outputIndex, int uid, int (*action)(snd_seq_t*, snd_seq_port_subscribe_t*),
                                     const char* actionName) {
    snd_seq_t* seq = mHandle;
    snd_seq_client_info_t* cinfo;
    snd_seq_port_subscribe_t* subs;
...
int initMIDI(int numIn, int numOut) {
    ...
    for (i = 0; i < numIn; i++) {
        ...
        port = snd_seq_create_simple_port(client->mHandle, str, SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_SUBS_WRITE,
    ...
    for (i = 0; i < numOut; i++) {
        ...
        port = snd_seq_create_simple_port(client->mHandle, str, SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ,
                                          SND_SEQ_PORT_TYPE_APPLICATION);
